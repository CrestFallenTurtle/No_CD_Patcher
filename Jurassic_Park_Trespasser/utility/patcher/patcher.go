...
func Begin_patch(file_path_exe string, file_path_smks string, file_path_other string) {
	if runtime.GOOS != "windows" {
		notify.Error("This tool works only on windows", "patcher.Begin_patch", 1)
	}
...
func internal_hash(path_to_file string) string {
	...
	if err != nil {
		notify.Error(err.Error(), "patcher.internal_hash", 1)
	}
...
func check_exe() {
	if strings.Contains(c_file.path_to_exe, EXE_NAME) { // This is what the exe is called
		...
		if hash_sum != EXE_HASH {
			notify.Error("Hash values for the exe didn't match. Assuming it's not the correct exe.", "patcher.check_exe", 1)
		}
	} else {
		notify.Error("The provided file '"+c_file.path_to_exe+"' doesn't seem to the target exe '"+EXE_NAME+"'", "patcher.read_file()", 1)
	}
}
...
func check_smk() {
	...
	if err != nil {
		notify.Error(err.Error(), "patcher.check_smk()", 1)
	}
	...
	if hash_smk["credits.smk"] != CREDITS_HASH ||
		...
		notify.Error("One or more hashes for the smks was wrong", "patcher.check_smk()", 1)
	}
}
...
func create_dir() {
	...
	if err != nil {
		notify.Error(err.Error(), "patcher.create_dir()", 1)
	}
}
...
func copy_credits() {
	...
	if err != nil {
		notify.Error(err.Error(), "patcher.copy_credits()", 1)
	}
...
func copy_newgame() {
	...
	if err != nil {
		notify.Error(err.Error(), "patcher.copy_newgame()", 1)
	}
...
func copy_tpassintro() {
	...
	if err != nil {
		notify.Error(err.Error(), "patcher.copy_tpassintro()", 1)
	}
...
func copy_win() {
	...
	if err != nil {
		notify.Error(err.Error(), "patcher.copy_win()", 1)
	}
...
func copy_other() {
	...
	if err != nil {
		notify.Error(err.Error(), "patcher.copy_other()", 1)
	}
...
func regedit() {
	...
	if err != nil {
		notify.Error(err.Error(), "patcher.regedit", 1)
	}
