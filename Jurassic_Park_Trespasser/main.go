...
func main() {
	...
	arg.Argument_add("--roll_back", "-rb", true, "Path to Trespasser exe to patch rollback any changes made", []string{"NULL"})
	arg.Argument_parse()

	if len(os.Args) > 1 {
		if arg.Argument_check("-h") { // Print help screen
			arg.Argument_help()
		} else if arg.Argument_check("-rb") { // Rollback changes
			patcher.Begin_rollback(arg.Argument_get("-rb"))
		} else {
